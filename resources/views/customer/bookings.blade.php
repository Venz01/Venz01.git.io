                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="flex items-center gap-2 mb-2">
                                            <div class="flex text-lg">{!! $booking->review->getStarDisplayAttribute() !!}</div>
                                            <span class="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700
                                                @if($booking->review->admin_status === 'approved') bg-green-100 text-green-800
                                                @elseif($booking->review->admin_status === 'under_review') bg-yellow-100 text-yellow-800 @endif">
                                                {{ $booking->review->admin_status === 'approved' ? 'Approved' : 'Pending' }}
                                            </span>
                                        </div>
                                    </div>
                                    @elseif($booking->booking_status === 'completed' && !$booking->review)
                                    <div class="space-y-3">
                                        <h4 class="font-semibold text-gray-900 dark:text-white text-xs">Review</h4>
                                        <div class="p-3 bg-amber-50 dark:bg-amber-900/20 border-2 border-dashed border-amber-200 dark:border-amber-800 rounded-lg text-center">
                                            ...
                                            <p class="text-sm font-medium text-amber-800 dark:text-amber-200">Leave a review</p>
                                            <a href="{{ route('customer.review.create', $booking->id) }}" class="text-xs text-amber-700 hover:text-amber-900 underline mt-1 block dark:text-amber-300">Write review →</a>
                                        </div>
                                    </div>
                                    @endif
