    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="relative bg-gradient-to-br from-green-700 via-green-600 to-emerald-500 rounded-3xl p-8 mb-8 text-white overflow-hidden">
                ...
                <div class="absolute -bottom-16 -left-8 w-80 h-80 bg-black/10 rounded-full"></div>

                <div class="relative z-10">
                    <div class="max-w-2xl mb-6">
                        <h1 class="text-3xl lg:text-4xl font-bold mb-2 tracking-tight">Find Your Perfect Package</h1>
                    ...
                    <form method="GET" action="{{ route('customer.caterers') }}" id="searchForm">
                        <div class="flex flex-col sm:flex-row gap-3">
                            <div class="flex-1 relative">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    ...
                                    class="w-full pl-12 pr-4 py-3.5 rounded-xl text-gray-900 dark:text-white dark:bg-gray-800 border-0 focus:ring-2 focus:ring-white/60 text-base shadow-lg placeholder-gray-400">
                            </div>

                            <button type="submit"
                                class="bg-white text-green-700 px-6 py-3.5 rounded-xl font-semibold hover:bg-green-50 active:bg-green-100 transition-colors flex items-center justify-center gap-2 shadow-lg shrink-0">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                                </svg>
                                Search
                            ...
                            <button type="button" id="filterToggle"
                                class="bg-green-800/60 hover:bg-green-800/80 border border-white/20 text-white px-5 py-3.5 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 shrink-0">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @auth
                                @if(auth()->user()->isCustomer())
                                    <button type="button" onclick="openDietaryPanel()" id="dietaryToggleBtn"
                                        class="relative bg-white/10 hover:bg-white/20 border border-white/30 hover:border-white/50 text-white px-5 py-3.5 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 shrink-0">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                        </svg>
                                        ...
                                        @if(auth()->user()->hasDietaryPreferences())
                                            <span class="absolute -top-1.5 -right-1.5 w-4 h-4 bg-yellow-400 rounded-full border-2 border-green-700 flex items-center justify-center text-green-900 font-bold" style="font-size:8px">
                        ...
                        <div id="advancedFilters" class="hidden mt-4 p-5 bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20">
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                                <div>
                                    <label class="block text-xs font-semibold text-green-100 mb-1.5 uppercase tracking-wider">Location (Bukidnon)</label>
                                    <select name="location" class="w-full px-3 py-2.5 rounded-lg text-gray-900 text-sm border-0" style="background-image:url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2216%22 height=%2216%22 viewBox=%220 0 24 24%22><path fill=%22%23374151%22 d=%22M7 10l5 5 5-5z%22/></svg>');background-repeat:no-repeat;background-position:right 10px center;padding-right:2rem;">
                                        <option value="">-- Select Municipality --</option>
                                        <option value="Malaybalay" {{ request('location') == 'Malaybalay' ? 'selected' : '' }}>Malaybalay (Capital City)</option>
                                        <option value="Valencia" {{ request('location') == 'Valencia' ? 'selected' : '' }}>Valencia</option>
                                        <option value="Maramag" {{ request('location') == 'Maramag' ? 'selected' : '' }}>Maramag</option>
                                        <option value="Manolo Fortich" {{ request('location') == 'Manolo Fortich' ? 'selected' : '' }}>Manolo Fortich</option>
                                        <option value="Quezon" {{ request('location') == 'Quezon' ? 'selected' : '' }}>Quezon</option>
                                        <option value="Don Carlos" {{ request('location') == 'Don Carlos' ? 'selected' : '' }}>Don Carlos</option>
                                        <option value="Kitaotao" {{ request('location') == 'Kitaotao' ? 'selected' : '' }}>Kitaotao</option>
                                        <option value="Cabanglasan" {{ request('location') == 'Cabanglasan' ? 'selected' : '' }}>Cabanglasan</option>
                                        <option value="Dangcagan" {{ request('location') == 'Dangcagan' ? 'selected' : '' }}>Dangcagan</option>
                                        <option value="Kibawe" {{ request('location') == 'Kibawe' ? 'selected' : '' }}>Kibawe</option>
                                        <option value="Pangantucan" {{ request('location') == 'Pangantucan' ? 'selected' : '' }}>Pangantucan</option>
                                        <option value="Talakag" {{ request('location') == 'Talakag' ? 'selected' : '' }}>Talakag</option>
                                        <option value="Lantapan" {{ request('location') == 'Lantapan' ? 'selected' : '' }}>Lantapan</option>
                                        <option value="Baungon" {{ request('location') == 'Baungon' ? 'selected' : '' }}>Baungon</option>
                                        <option value="Libona" {{ request('location') == 'Libona' ? 'selected' : '' }}>Libona</option>
                                        <option value="Sumilao" {{ request('location') == 'Sumilao' ? 'selected' : '' }}>Sumilao</option>
                                        <option value="Impasugong" {{ request('location') == 'Impasugong' ? 'selected' : '' }}>Impasugong</option>
                                    </select>
                                </div>
                                <div>
