{{--
    notifications/_icon.blade.php
    Usage: @include('notifications._icon', ['icon' => $notification->icon])

    REPLACES the giant @switch block in the original index.blade.php.
    All SVGs are identical to the originals — just moved here so the
    main view stays readable and the switch only compiles once per request.
--}}
@php
    $icons = [
    ...
    $path = $icons[$icon] ?? $icons['bell'];
@endphp
<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $path !!}</svg>